...
function clicked(event)
{
	...
	if(!startSelected)
	{
		...
		startSelected = true;
		this.innerHTML = '<i class="fas fa-running fa-2x" style="color:rgba(0,0,0,0.75);"></i>';
	...
	else if(startSelected && !endSelected)
	{
		...
		this.innerHTML = '<i class="fa fa-map-marker fa-lg" aria-hidden="true" style="color:rgba(0,0,0,0.75);"></i>';
	}
	else
	{
		if(node==endPoint)
...
function generateRandom()
{
	...
	while(count<percentageWalls)
	{
		...
		if(thisValueAlreadyCame[randomCell] || randomCell==startPoint || randomCell==endPoint)
			continue;
		count++;
...
{
	if(neighbor<0 || neighbor>=n*18)
		return false;

	if(isWall[neighbor]==true || isVisited[neighbor]==true)
		return false;
...
function dijkstraAlgo(st, nd)
{
	...
	while(NotFound==false)
	{
		...
		var neighbors = [presentNode+1, presentNode+n, presentNode-1, presentNode-n];

		for(let i=0;i<4;i++)
		{
			if(isVisited[neighbors[i]]==false && check(neighbors[i], presentNode)==true && (distance[presentNode]+1)<distance[neighbors[i]])
			{
